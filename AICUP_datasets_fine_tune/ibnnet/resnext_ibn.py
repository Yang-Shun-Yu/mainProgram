...
class ResNeXt_IBN(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, ibn=None):
        ...
        layers.append(block(self.inplanes, planes, self.baseWidth,
                            self.cardinality, stride, downsample, None if ibn == 'b' else ibn))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, self.baseWidth,
                                self.cardinality, 1, None, None if (ibn == 'b' and i < blocks-1) else ibn))
...
def resnext101_ibn_b(pretrained=False, baseWidth=4, cardinality=32):
    model = ResNeXt_IBN(baseWidth, cardinality, [3, 4, 23, 3], ('b', 'b', None, None))
    return model